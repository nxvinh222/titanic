...
df.Survived.value_counts(normalize=True).plot(kind="bar", alpha = 0.5, color = 'b')
plt.title("Survived")
...
df.Sex[df.Survived == 1].value_counts(normalize=True).plot(kind="bar", alpha = 0.5, color = [female_color, 'blue'])
plt.title("Sex of Survived")

plt.subplot2grid((3,4), (1,0), colspan=4)
for x in [1,2,3]:
    df.Survived[df.Pclass == x].plot(kind="kde") #filter
plt.title("Class wrt Survided")
